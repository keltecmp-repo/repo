        ...
        try:
            ...
            if not id:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}] Notificação: Não formatado corretamente[/COLOR]".format(CONFIG.COLOR2))
                return
            window.show_notification(msg, test=True)
        except Exception as e:
            logging.log("Erro na janela de notificações: {0}".format(str(e)), level=xbmc.LOGERROR)
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}] URL inválido para notificação[/COLOR]".format(CONFIG.COLOR2))
