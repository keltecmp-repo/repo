...
def remove_current():
    ...
    ok = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Tem certeza de que deseja remover o advancedsettings.xml atual?[/COLOR]".format(CONFIG.COLOR2),
    ...
    if ok:
        if os.path.exists(CONFIG.ADVANCED):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                               "[COLOR {0}]advancedsettings.xml removido[/COLOR]".format(CONFIG.COLOR2))
            xbmc.executebuiltin('Container.Refresh()')
        else:
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                               "[COLOR {0}]advancedsettings.xml não encontrado[/COLOR]".format(CONFIG.COLOR2))
    else:
        logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                               "[COLOR {0}]advancedsettings.xml não removido[/COLOR]".format(CONFIG.COLOR2))
...
def _write_setting(category, tag, value):
    ...
    logging.log('Escrevendo {0} - {1}: {2} para advancedsettings.xml'.format(category, tag, value), level=xbmc.LOGDEBUG)
...
class AdvancedMenu:
    ...
    def quick_configure(self):
        directory.add_file('Mudanças não serão refletidas até que o Kodi seja reiniciado.', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    def show_section(self, tags):
        ...
        if len(self.tags) == 0:
            directory.add_file('Nenhuma configuração para esta categoria existe em seu arquivo advancedsettings.xml atual.', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    def _network(self, tag, current):
        msgs = {'curlclienttimeout': 'Tempo limite em segundos para conexões libcurl (http / ftp)',
                'curllowspeedtime': 'Tempo em segundos para libcurl considerar uma conexão de baixa velocidade',
                'curlretries': 'Quantidade de tentativas para certas operações libcurl com falha (por exemplo, tempo limite)',
                'httpproxyusername': 'Nome de usuário para Basic Proxy Authentication',
                'httpproxypassword': 'Senha para Basic Proxy Authentication'}
