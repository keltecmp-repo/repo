            ...
            for root, dirs, files in os.walk(CONFIG.PACKAGES):
                ...
                if file_count > 0:
                    ...
                    logging.log_notify(CONFIG.ADDONTITLE,
                              ...
                              '[COLOR {0}]Pacotes claros: Nenhum encontrado![/COLOR]'.format(CONFIG.COLOR2))

        except Exception as e:

            logging.log_notify(CONFIG.ADDONTITLE,

                      '[COLOR {0}]Pacotes claros: Erro![/COLOR]'.format(CONFIG.COLOR2))

            logging.log("Erro ao limpar pacotes: {0}".format(str(e)), level=xbmc.LOGERROR)
        ...
        try:
            ...
            if file_count > 0:
                ...
                logging.log_notify(CONFIG.ADDONTITLE,
                          ...
                          '[COLOR {0}]Pacotes claros: sucesso: {1}[/COLOR]'.format(CONFIG.COLOR2, tools.convert_size(cleanupsize)))
...
def clear_archive():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        ...
                        '[COLOR {0}]Deseja limpar \'Archive_Cache\' pasta?[/COLOR]'.format(CONFIG.COLOR2),

                        nolabel='[B][COLOR red]Não, Cancelar[/COLOR][/B]',

                        yeslabel='[B][COLOR springgreen]Sim claro[/COLOR][/B]'):
...
def clear_function_cache(over=False):
    ...
    if not over:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            nolabel='[B][COLOR red]Não, Cancelarl[/COLOR][/B]',
...
def clear_crash():
    ...
    if len(files) > 0:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            nolabel="[B][COLOR red]Guardar Logs[/COLOR][/B]"):
...
def clear_thumbs(type=None):
    ...
    if type is not None:
        ...
        choice = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Gostaria de excluir o {1} e as pastas de miniaturas relacionadas?'.format(CONFIG.COLOR2, latest) + '\n' + "Eles irão repovoar na próxima inicialização[/COLOR]", nolabel='[B][COLOR red]Não exclua[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Apagar Thumbs[/COLOR][/B]')
    ...
    if choice == 1:
        ...
        try:
            ...
            logging.log('Falha ao excluir, Purging DB.')
...
def remove_addon(addon, name, over=False, data=True):
    ...
    if over:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                               ...
                               '[COLOR {0}]Tem certeza de que deseja excluir o complemento:'.format(CONFIG.COLOR2)
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            yeslabel='[B][COLOR springgreen]Remover Dados[/COLOR][/B]',
            ...
            logging.log_notify('[COLOR {0}]Remover Dados Adicionais[/COLOR]'.format(CONFIG.COLOR1),
        ...
        if addon in CONFIG.EXCLUDES:
            ...
            logging.log_notify("[COLOR {0}]Plugin protegido[/COLOR]".format(CONFIG.COLOR1),
                               ...
                               "[COLOR {0}]Não tem permissão para remover dados adicionais[/COLOR]".format(CONFIG.COLOR2))

        elif os.path.exists(addon_data):

            if dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Você também gostaria de remover os dados adicionais para:[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, addon), yeslabel='[B][COLOR springgreen]Remover Dados[/COLOR][/B]', nolabel='[B][COLOR red]Não Remover[/COLOR][/B]'):
                ...
                try:
                    ...
                    logging.log("Erro ao excluir: {0}".format(addon_data))

            else:

                logging.log('Dados adicionais para {0} não foi removido'.format(addon))
...
def remove_addon_menu():
    ...
    if len(addonnames) == 0:
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           ...
                           "[COLOR {0}]Não há complementos para remover[/COLOR]".format(CONFIG.COLOR2))

        return

    selected = dialog.multiselect("{0}: Selecione os complementos que deseja remover.".format(CONFIG.ADDONTITLE), addonnames)
