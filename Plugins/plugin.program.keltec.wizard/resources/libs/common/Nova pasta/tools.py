...
if sys.version_info[0] > 2:
    ...
    def open(file, mode='r', buffering=-1, encoding='utf-8', errors=None, newline=None, closefd=True, opener=None):
        if newline is not None:
            warnings.warn('newline não é compatível com py2')
        if not closefd:
            warnings.warn('closefd não é compatível com py2')
        if opener is not None:
            warnings.warn('opener não é compatível com py2')
...
def remove_folder(path):
    ...
    logging.log("Apagando pasta: {0}".format(path))
    try:
...
def remove_file(path):
    ...
    logging.log("Excluindo Arquivo: {0}".format(path))
    try:
        os.remove(path)
    except:
        return False
...
def empty_folder(folder):
    ...
    for root, dirs, files in os.walk(folder, topdown=True):
        ...
        if file_count == 0:
            ...
            from resources.libs.common import logging
            logging.log("Pasta vazia: {0}".format(root))
    return total
            ...
            for f in files:
                try:
                    ...
                    try:
                        ...
                        logging.log("Erro ao excluir {0}".format(f), level=xbmc.LOGERROR)
            for d in dirs:
                total_folds += 1
                try:
                    ...
                    logging.log("Erro ao excluir {0}".format(d), level=xbmc.LOGERROR)
...
def ensure_folders(folder=None):
    ...
    try:
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                      "[COLOR {0}]Erro ao criar diretórios complementares:[/COLOR]".format(CONFIG.COLOR2)
...
def kill_kodi(msg=None, over=None):
    if over:
        ...
        if not msg:
            msg = '[COLOR {0}]Você está prestes a fechar o Kodi. Você gostaria de continuar?[/COLOR]'.format(CONFIG.COLOR2)
        
        choice = dialog.yesno('Forçar o fechamento do Kodi',
                                  msg,
                                  nolabel='[B][COLOR red] Não Cancelar[/COLOR][/B]',
                                  yeslabel='[B][COLOR springgreen]Forçar o fechamento do Kodi[/COLOR][/B]')
    if choice == 1:
        from resources.libs.common import logging
        logging.log("Forçar o fechamento do Kodi: plataforma[{0}]".format(str(platform())))
        os._exit(1)
...
def convert_special(url, over=False):
    ...
    start = 0
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Mudando os caminhos físicos para especiais".format(CONFIG.COLOR2) + "\n" + "Por favor, aguarde[/COLOR]")
    for root, dirs, files in os.walk(url):
        for file in files:
            start += 1
            perc = int(percentage(start, total))
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                progress_dialog.update(perc, "[COLOR {0}]Varrendo: [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, root.replace(CONFIG.HOME, '')) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, file) + '\n' + "Por favor, aguarde[/COLOR]")
                ...
                try:
                    ...
                    logging.log('Incapaz de abrir o arquivo para converter caminhos especiais: {}'.format(os.path.join(root, file)))

                if progress_dialog.iscanceled():
                    progress_dialog.close()
                    logging.log_notify(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]Converter caminho cancelado[/COLOR]".format(CONFIG.COLOR2))
                    sys.exit()
    progress_dialog.close()
    logging.log("[Converter caminhos em especiais] Completo")
    if not over:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Converter caminhos em especiais: Completo![/COLOR]".format(CONFIG.COLOR2))
...
def reload_fix(default=None):
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]AVISO: Às vezes, recarregar o perfil causa o travamento do Kodi. Enquanto Kodi estiver recarregando o perfil, não pressione nenhum botão![/COLOR]".format(CONFIG.COLOR2))
...
def ascii_check(use=None, over=False):
    ...
    if use is None:
        source = dialog.browse(3,
                                   '[COLOR {0}]Selecione a pasta que deseja verificar[/COLOR]'.format(CONFIG.COLOR2),
                                   'files', '', False, False, CONFIG.HOME)
        if over:
            yes = 1
        else:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Você quer [COLOR {1}]excluir[/COLOR] todos os nomes de arquivos com caracteres especiais ou você prefere apenas [COLOR {2}]escanear e ver[/COLOR] os resultados no log?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
    ...
    if source == "":
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Verificação ASCII: Cancelado[/COLOR]".format(CONFIG.COLOR2))
        return
    ...
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            progress_dialog.update(prog2, "[COLOR {0}]Verificando arquivos não ASCII".format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file) + '\n' + 'Por favor, aguarde[/COLOR]')
            try:
                file.encode('ascii')
            except UnicodeEncodeError:
                logging.log("[ASCII Check] Caráter ilegal encontrado no arquivo: {0}".format(file))
                ...
                if yes:
                    try:
                        ...
                        afiles.write('\n')
                        f1 += 1
                        logging.log("[ASCII Check] Arquivo Removido: {0} ".format(badfile), level=xbmc.LOGERROR)
                    except:
                        for chunk in chunks(badfile, 75):
                            afails.write(chunk+'\n')
                        afails.write('\n')
                        f2 += 1
                        logging.log("[ASCII Check] Arquivo falhou: {0} ".format(badfile), level=xbmc.LOGERROR)
                else:
                    for chunk in chunks(badfile, 75):
                        afiles.write(chunk+'\n')
                    afiles.write('\n')
                    f1 += 1
                    logging.log("[ASCII Check] Arquivo encontrado: {0} ".format(badfile), level=xbmc.LOGERROR)
                pass
        if progress_dialog.iscanceled():
            progress_dialog.close()
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Verificação ASCII cancelada[/COLOR]".format(CONFIG.COLOR2))
            sys.exit()
    progress_dialog.close()
    afiles.close()
    afails.close()
    ...
    if total > 0:
        ...
        if yes:
            if use:
                logging.log_notify(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]Verificar ASCII: {1} Removido / {2} Fracassado.[/COLOR]".format(CONFIG.COLOR2, f1, f2))
            else:
                window.show_text_box("Visualizando arquivos ASCII removidos",
                                  "[COLOR yellow][B]{0} Arquivos Removidos:[/B][/COLOR]\n {1}\n\n[COLOR yellow][B]{2} Arquivos com falha:[B][/COLOR]\n {3}".format(f1, msg, f2, msg2))
        else:
            window.show_text_box("Visualizando arquivos ASCII encontrados", "[COLOR yellow][B]{0} Arquivos encontrados:[/B][/COLOR]\n {1}".format(f1, msg))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Verificação ASCII: Nenhum encontrado.[/COLOR]".format(CONFIG.COLOR2))
    ...
    if _is_url(url):
        try:
            ...
            if response.status_code < 300:
                logging.log("A verificação de URL foi aprovada para {0}: Código de status [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return True
            elif response.status_code < 400:
                logging.log("Verificação de URL redirecionada de {0} to {1}: Código de status [{2}]".format(url, response.headers['Location'], response.status_code), level=xbmc.LOGDEBUG)
                return _check_url(response.headers['Location'])
            elif response.status_code == 401:
                logging.log("URL requer autenticação para {0}: Código de status [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return 'auth'
            else:
                logging.log("A verificação de URL falhou para {0}: Código de status [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return False
        except Exception as e:
            logging.log("Erro de verificação de URL for {0}: [{1}]".format(url, e), level=xbmc.LOGDEBUG)
            return False
    else:
        logging.log("URL não é de um esquema válido: {0}".format(url), level=xbmc.LOGDEBUG)
        return False
...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        if response.status_code == 401:
            retry = dialog.yesno(CONFIG.ADDONTITLE, 'O nome de usuário ou a senha eram inválidos. Você gostaria de tentar de novo?', yeslabel='Tente novamente', nolabel='Cancelar')
            ...
            if retry and count < 3:
                ...
                dialog.ok(CONFIG.ADDONTITLE, 'Autenticação Fracassado.')
                return False
