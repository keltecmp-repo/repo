...
def upload_log():
    ...
    for item in files:
        ...
        if filetype == 'log':
            ...
            error = "Erro ao postar o {0} file".format(name)
            ...
            name = "crash log"
            error = "Erro ao postar o arquivo de crashlog"
        succes, data = read_log(item[1])
        if succes:
            content = clean_log(data)
            succes, result = post_log(content, name)
            if succes:
                msg = "Poste este url ou escaneie o código QR para o seu [COLOR {0}]{1}[/COLOR]," \
                      "junto com uma descrição do problema:[CR][COLOR {2}]{3}[/COLOR]".format(
...
def get_files():
    ...
    if kodilog:
        if os.path.exists(kodilog):
            ...
            show_result("Nenhum arquivo de log encontrado")
    else:
        show_result("Nenhum arquivo de log encontrado")
    if CONFIG.KEEPOLDLOG:
        if old:
            if os.path.exists(old):
                ...
                show_result("Nenhum arquivo de log antigo encontrado")
        else:
            show_result("Nenhum arquivo de log antigo encontrado")
    if CONFIG.KEEPWIZLOG:
        if wizard:
            ...
            show_result("Nenhum arquivo de log do assistente encontrado")
    if CONFIG.KEEPCRASHLOG:
        ...
        if xbmc.getCondVisibility('system.platform.osx'):
            ...
            log("Não há suporte para logs de travamento do Windows, desative esta opção nas configurações do complemento")
        elif tools.platform() == 'android':
            log("Não há suporte para crashlogs do Android, desative esta opção nas configurações do complemento")
        ...
        if len(items) == 0:
            log("Nenhum arquivo de crashlog encontrado")
    return logfiles
...
def read_log(path):
    try:
        ...
        log('arquivo está vazio: {0}'.format(e))
        return False, "Incapaz de ler o arquivo"
...
def post_log(data, name):
    ...
    try:
        ...
        a = 'falhou ao conectar com o servidor'
    ...
    try:
        ...
        a = 'incapaz de recuperar o url da pasta'
        log("{0}: {1}".format(a, str(e)), level=xbmc.LOGERROR)
        return False, a
...
def view_log_file():
    ...
    if len(choices) > 0:
        ...
        if which == -1:
            log_notify('[COLOR {0}]Ver Log[/COLOR]'.format(CONFIG.COLOR1),
                       '[COLOR {0}]Ver Log Cancelled![/COLOR]'.format(CONFIG.COLOR2))
            return
        elif which == 0:
            logtype = mainlog
        elif which == 1:
        ...
        log_notify('[COLOR {0}]Ver Log[/COLOR]'.format(CONFIG.COLOR1),
                   '[COLOR {0}]Nenhum arquivo de log encontrado![/COLOR]'.format(CONFIG.COLOR2))
        return
    else:
        if mainlog:
            logtype = mainlog
    ...
    window.show_log_viewer("Visualizando Arquivo de Log", log_file=logtype, ext_buttons=True)
...
def swap_debug():
    ...
    log("Registro de depuração Get Settings: {0}".format(str(response)))
    if 'false' in response:
        ...
        log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]Registro de depuração:[/COLOR] [COLOR {1}]Habilitado[/COLOR]'.format(CONFIG.COLOR1,
                                                                                                   CONFIG.COLOR2))
        log("Registro de depuração Set Settings: {0}".format(str(response)))
        ...
        response = xbmc.executeJSONRPC(query)
        log_notify(CONFIG.ADDONTITLE,
                   '[COLOR {0}]Registro de depuração:[/COLOR] [COLOR {1}]Disabled[/COLOR]'.format(CONFIG.COLOR1,
                                                                                         CONFIG.COLOR2))
        log("Registro de depuração Set Settings: {0}".format(str(response)))
...
def error_checking(log=None, count=None, last=None):
    ...
    if log is None:
        ...
        if not old  and not curr:
            if count is None:
                log_notify('[COLOR {0}]Ver log de erros/COLOR]'.format(CONFIG.COLOR1),
                           '[COLOR {0}]Nenhum arquivo de log encontrado![/COLOR]'.format(CONFIG.COLOR2))
                return
            else:
                return 0
    ...
    if count is not None:
        ...
        if last is None:
            ...
            for item in errors:
                i += 1
                string += "[B][COLOR red]NÚMERO DE ERRO {0}:[/B][/COLOR] {1}\n".format(str(i), item.replace(CONFIG.HOME, '/').replace('                                        ', ''))
            window.show_log_viewer("Visualizando Erros no Log", string)
        else:
            string = "[B][COLOR red]Último erro no log:[/B][/COLOR] {0}\n".format(errors[0].replace(CONFIG.HOME, '/').replace('                                        ', ''))
            window.show_log_viewer("Viewing Last Error in Log", string)

    else:
        log_notify('[COLOR {0}]Ver log de erros/COLOR]'.format(CONFIG.COLOR1),
                   '[COLOR {0}]Nenhum erro encontrado![/COLOR]'.format(CONFIG.COLOR2))
