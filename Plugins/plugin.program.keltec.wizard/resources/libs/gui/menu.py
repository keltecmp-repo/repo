...
def check_for_fm():
    ...
    if fm == 0 and 'com.android.documentsui' not in apps:
        ...
        choose = dialog.yesno(CONFIG.ADDONTITLE, 'Parece que seu dispositivo não tem gerenciador de arquivos padrão. Você gostaria de definir um agora?')

        if not choose:

            dialog.ok(CONFIG.ADDONTITLE, 'Se um APK for baixado, mas não abrir para instalação, tente alterar o gerenciador de arquivos em {}\'s "Configurações de instalação".'.format(CONFIG.ADDONTITLE))
...
def run_speed_test():
    ...
    try:
        ...
        logging.log("[Teste de velocidade] Erro ao executar o teste de velocidade: {0}".format(e), level=xbmc.LOGDEBUG)
...
def system_info():
    ...
    directory.add_file('[COLOR {0}]Versão:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, data[1]), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    directory.add_file('[COLOR {0}]Tempo de atividade atual:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, data[6]), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)

    directory.add_file('[COLOR {0}]Tempo de atividade total:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, data[7]), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    ...
    directory.add_file('[COLOR {0}]Armazenamento grátis:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, storage_free), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
...
def save_menu():
    ...
    on = '[COLOR springgreen]Ligado[/COLOR]'
    ...
    for folder in sorted(fold, key=lambda x: x):
        ...
        if os.path.exists(xml):
            ...
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(addonid)):
                ...
                state = "[COLOR springgreen][Habilitado][/COLOR]"

                goto = "false"

            else:

                state = "[COLOR red][Desabilitado][/COLOR]"
...
def change_freq():
    ...
    change = dialog.select("[COLOR {0}]Com que frequência você listaria Limpeza automática na inicialização?[/COLOR]".format(CONFIG.COLOR2), CONFIG.CLEANFREQ)
    ...
    if not change == -1:
        ...
        logging.log_notify('[COLOR {0}]Limpeza Automática[/COLOR]'.format(CONFIG.COLOR1),
                           ...
                           '[COLOR {0}]Frequência Agora {1}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.CLEANFREQ[change]))
