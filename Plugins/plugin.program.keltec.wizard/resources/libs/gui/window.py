...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    ...
    msgcurrent = 'Executando a versão mais recente da compilação instalada: '

    msgupdate = 'Update available for installed build: '

    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)

    current_version = 'Versão Atual: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)

    latest_version = 'Última versão: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
