...
class BuildMenu:
    ...
    def view_build(self, name):
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            directory.add_file('Informações de construção', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
            ...
            if previewcheck:
                ...
                directory.add_file('Ver visualização do vídeo', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
            ...
            if versioncheck:
                ...
                directory.add_file(
                    ...
                    '[I]Construir projetado para Kodi v{0} (instalado: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
            ...
            directory.add_file('INSTALAR', {'mode': 'install', 'action': 'build', 'name': name}, description=description, fanart=fanart,
            ...
            if guicheck:
                ...
                directory.add_file('Aplicar guiFix', {'mode': 'install', 'action': 'gui', 'name': name}, description=description, fanart=fanart,
    ...
    def build_info(self, name):
        ...
        if response:
            ...
            if check.check_build(name, 'url'):
                ...
                msg = "[COLOR {0}]Nome da construção:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, name)
                ...
                msg += "[COLOR {0}]Versão Kodi:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, kodi)

                msg += "[COLOR {0}]Conteúdo adulto:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, adult)

                msg += "[COLOR {0}]Descrição:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, description)



                if extend:

                    msg += "[COLOR {0}]Última atualização:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, created)

                    msg += "[COLOR {0}]Tamanho Extraído:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(extracted))))

                    msg += "[COLOR {0}]Tamanho Zip:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(zipsize))))

                    msg += "[COLOR {0}]Nome da Skin:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, skin)

                    msg += "[COLOR {0}]Programa:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, programs)
                    ...
                    msg += "[COLOR {0}]Fotos:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, picture)

                    msg += "[COLOR {0}]Repositórios:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, repos)

                    msg += "[COLOR {0}]Scripts:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, scripts)

                    msg += "[COLOR {0}]Binários:[/COLOR] [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, binaries)



                window.show_text_box("Visualizando informações de compilação: {0}".format(name), msg)
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('Nova instalação'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID, buildname)))

        menu_items.append((CONFIG.THEME3.format('Instalação normal'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=normal)'.format(CONFIG.ADDON_ID, buildname)))

        menu_items.append((CONFIG.THEME3.format('Aplicar guiFix'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=gui)'.format(CONFIG.ADDON_ID, buildname)))

        menu_items.append((CONFIG.THEME3.format('Informações de construção'), 'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))

        menu_items.append((CONFIG.THEME2.format('{0} Configurações'.format(CONFIG.ADDONTITLE)), 'RunPlugin(plugin://{0}/?mode=settings)'.format(CONFIG.ADDON_ID)))
