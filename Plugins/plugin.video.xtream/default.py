...
#########################=XC VARIABLES=#####################################
dns				  = control.setting('DNS') or ''
username		  = control.setting('Username') or ''
password		  = control.setting('Password') or ''
...
def home():
	...
	tools.addDir('Pesquisar','url',5,iconsearch,background,'')    
...
def series_seasons(url):
	...
	for ser in ser_cat['episodes']:
		...
		try:
			...
			background = ''
		tools.addDir('Temporada - '+ser,url+'&season_number='+str(ser),20,thumb,background,'')
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		if 'ON' in description:
			pas = tools.keypopup('Coloque a Senha dos Adultos:')
...
def accountinfo():
	...
	tools.addDir('[B][COLOR white]Data Expiração:[/COLOR][/B] '+expiry,'','',icon,background,'')
	tools.addDir('[B][COLOR white]Status da Conta :[/COLOR][/B] %s'% parse['user_info']['status'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Conexões atuais:[/COLOR][/B] '+ parse['user_info']['active_cons'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Conexões permitidas:[/COLOR][/B] '+ parse['user_info']['max_connections'],'','',icon,background,'')
