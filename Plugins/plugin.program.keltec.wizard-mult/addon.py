...
def MainMenu():
	addDir('[COLOR gold]Instalar Wizard Builds KelTec Media Play [/COLOR]','',1,addon_icon,addon_fanart,local_string(30001),isFolder=True)
#	addDir('Mantenimiento','',5,addon_icon,addon_fanart,local_string(30002),isFolder=True)
#	addDir('Apagar TODO','',4,addon_icon,addon_fanart,local_string(30003),isFolder=False)
...
def subMenu_maintenance():
	addDir('Limpar Packages','',6,addon_icon,addon_fanart,local_string(30005),isFolder=False)
	addDir('Limpar Capas','',7,addon_icon,addon_fanart,local_string(30008),isFolder=False)
...
def main(NAME, NAME2, VERSION, URL, ICON, FANART, DESCRIPTION):
	...
	yesInstall = dialog.yesno(NAME, 'Vou instalar o Wizard KelTec Media Play', nolabel='Cancelar', yeslabel='Continuar')
	if yesInstall:
	    save_check()
	    save_move1()
	    yesFresh = dialog.yesno('Apagar TUDO', 'Recomendado excluir tudo antes de instalar a Build.', nolabel='Não', yeslabel='Apagar')
	    if yesFresh:
	    	freshStart()
...
def freshStart():
	...
	if yesFresh:
		...
		dp.create(addon_name, 'Apagar Capas...')
		xbmc.sleep(1000)
		dp.update(30, 'Apagar Capas...')
		xbmc.sleep(1000)
		...
		if not os.path.exists(packages):
			os.mkdir(packages)
		dp.update(100, 'Apagar Capas...Feito')
		xbmc.sleep(2000)
		if mode == 4:
			save_move2()
			...
			dialog.ok(addon_name, 'Terminamos. Forçar o Kodi a terminar.')
			os._exit(1)
	else:
		return
...
def buildInstall(NAME, NAME2, VERSION, URL):
	...
	dp.create(NAME + ' - ' + str(length2) + ' MB', 'Descarregando Build...')
	dp.update(0, 'Descarregando Build...')
	#
	if length:
		...
		while True:
			...
			tempzip.write(buf)
			dp.update(size2, 'Descarregando Build...' + str(size2) + '/' + str(length2) + 'MB')
				
	else:
		dp.update(50, 'Descarregando Build...')
		tempzip.write(zipresp.read())
	if length:
		dp.update(100, 'Descarregando Build...' + str(size2) + '/' + str(length2) + 'MB...Feito!')
	else:
		dp.update(100, 'Descarregando Build...Feito!')
	xbmc.sleep(1000)      
	tempzip.close()
	dp.update(66, 'Extraindo...')
	xbmc.sleep(1000)
	zf = ZipFile(zippath)
	zf.extractall(path = home)
	dp.update(100, 'Extraindo...Feito!')
	xbmc.sleep(2000)
	zf.close()
	os.unlink(zippath)
	save_move2()
	...
	addon.setSetting('firstrun', 'true')
	dialog.ok(addon_name, 'Terminamos. Force o encerramento do Kodi.')
	os._exit(1)
