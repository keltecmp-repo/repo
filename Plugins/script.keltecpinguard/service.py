...
# Class to handle core Keltec Pin Guard behaviour
class KeltecPinGuard():
    pinLevelCached = 0
    pinLevelCacheExpires = -1

    @staticmethod
    def isKeltecPinGuardEnabled():
    ...
    @staticmethod
    def clearPinCached():
        log("KeltecPinGuard: Clearing Cached pin that was at level %d" % KeltecPinGuard.pinLevelCached)
        KeltecPinGuard.pinLevelCached = 0
    ...
    def setCachedPinLevel(level):
        ...
        if cacheDuration != 0:
            if KeltecPinGuard.pinLevelCached < level:
                log("KeltecPinGuard: Updating cached pin level to %d" % level)
                KeltecPinGuard.pinLevelCached = level
        # Check to see if the duration expires
        if cacheDuration > 0:
            KeltecPinGuard.pinLevelCacheExpires = int(time.time()) + (cacheDuration * 60)
        else:
            KeltecPinGuard.pinLevelCacheExpires = -1

    @staticmethod
    def getCachedPinLevel():
        # Check to see if the pin was only cached for a set time
        if KeltecPinGuard.pinLevelCacheExpires > 0:
            if int(time.time()) > KeltecPinGuard.pinLevelCacheExpires:
                # The cached time has expired, so reset the security
                KeltecPinGuard.pinLevelCacheExpires = -1
                KeltecPinGuard.pinLevelCached = 0
        return KeltecPinGuard.pinLevelCached
    ...
    def promptUserForPin(requiredLevel=1):
        ...
        # If user pressed Back/Cancel, allow them to navigate freely
        # They simply won't have access to protected content
        if numberpad.wasCancelled():
            log("KeltecPinGuard: User cancelled PIN entry - allowing free navigation")
            del numberpad
            lastActivityTime = time.time()
            return None  # None = cancelled (distinct from True=correct, False=wrong)
        ...
        if pinMatchLevel >= requiredLevel:
            log("KeltecPinGuard: Pin entered correctly for security level %d" % pinMatchLevel)
            userHasAccess = True
            KeltecPinGuard.setCachedPinLevel(pinMatchLevel)
        else:
            log("KeltecPinGuard: Incorrect Pin Value Entered, required level %d entered level %d" % (requiredLevel, pinMatchLevel))
            userHasAccess = False
    ...
    def displayInvalidPinMessage(level=1):
        ...
        if notifType == Settings.INVALID_PIN_NOTIFICATION_POPUP:
            ...
            if Settings.getNumberOfLevels() > 1:
                cmd = 'Notification("{0}", "{1} {2}", 3000, "{3}")'.format(ADDON.getLocalizedString(32104), ADDON.getLocalizedString(32211), str(level), ICON)
            else:
                cmd = 'Notification("{0}", "{1}", 3000, "{2}")'.format(ADDON.getLocalizedString(32001), ADDON.getLocalizedString(32104), ICON)
            xbmc.executebuiltin(cmd)
        elif notifType == Settings.INVALID_PIN_NOTIFICATION_DIALOG:
            msg = ADDON.getLocalizedString(32104)
            if Settings.getNumberOfLevels() > 1:
                msg = "%s — %s %d" % (msg, ADDON.getLocalizedString(32211), level)
...
class KeltecPinGuardMonitor(xbmc.Monitor):
    def onSettingsChanged(self):
        log("KeltecPinGuardMonitor: Notification of settings change received")
        Settings.reloadSettings()

    def onScreensaverActivated(self):
        log("KeltecPinGuardMonitor: Screensaver started, clearing cached pin")
        KeltecPinGuard.clearPinCached()


# Our Monitor class so we can find out when a video file has been selected to play
class KeltecPinGuardPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        log("KeltecPinGuardPlayer: Notification that something started playing")
        ...
        if xbmcgui.Window(10000).getProperty("VideoScreensaverRunning"):
            log("KeltecPinGuardPlayer: Detected VideoScreensaver playing")
            return

        # Check if the Pin is set, as no point prompting if it is not
        if not KeltecPinGuard.isKeltecPinGuardEnabled():
            return
        ...
        log("KeltecPinGuardPlayer: File being played is: %s" % filePath)
        ...
        if title not in [None, ""]:
            isTvShow = True
            log("KeltecPinGuardPlayer: TVShowTitle: %s" % title)
            pinDB = KeltecPinGuardDB()
            ...
            if title not in [None, ""]:
                if not isMusicVideo:
                    # Check for a Movie
                    log("KeltecPinGuardPlayer: Title: %s" % title)
                    pinDB = KeltecPinGuardDB()
                    securityLevel = pinDB.getMovieSecurityLevel(title)
                    del pinDB
                else:
                    # Now check to see if this is  music video
                    log("KeltecPinGuardPlayer: Checking Music video for: %s" % title)
                    pinDB = KeltecPinGuardDB()
        ...
        if securityLevel == -1:
            log("KeltecPinGuardPlayer: Security level is -1, so allowing access")
            return
        ...
        if securityLevel < 1 and Settings.isActiveFileSource() and Settings.isActiveFileSourcePlaying():
            ...
            for key in securityDetails.keys():
                if key in filePath:
                    ...
                    log("KeltecPinGuardPlayer: Setting path based security to %d" % securityLevel)
        ...
        if securityLevel < 1:
            ...
            if cert not in [None, ""]:
                log("KeltecPinGuardPlayer: Checking for certification restrictions: %s" % str(cert))
            ...
            if securityLevel < 1:
                if isTvShow:
                    if not Settings.isSupportedTvShowClassification(cert):
                        ...
                        log("KeltecPinGuardPlayer: Setting TV Show to level %d as there is no valid MPAA value" % securityLevel)
                    ...
                    if not Settings.isSupportedMovieClassification(cert):
                        ...
                        log("KeltecPinGuardPlayer: Setting Movie to level %d as there is no valid MPAA value" % securityLevel)
            ...
            if securityLevel > 0:
                ...
                if isBackgroundMedia:
                    securityLevel = 0
                    log("KeltecPinGuardPlayer: Playing background media")
        ...
        if securityLevel < 1:
            if title in [None, ""]:
                ...
                log("KeltecPinGuardPlayer: No security enabled, no title available")
            else:
                log("KeltecPinGuardPlayer: No security enabled for %s" % title)
            return

        # Check if we have already cached the pin number and at which level
        if KeltecPinGuard.getCachedPinLevel() >= securityLevel:
            log("KeltecPinGuardPlayer: Already cached pin at level %d, allowing access" % KeltecPinGuard.getCachedPinLevel())
            return
        ...
        if xbmcgui.Window(10000).getProperty("KeltecPinGuardPrompting"):
            log("KeltecPinGuardPlayer: Already prompting for security code")
            return

        # Set the flag so other threads know we are processing this play request
        xbmcgui.Window(10000).setProperty("KeltecPinGuardPrompting", "true")
        ...
        log("KeltecPinGuardPlayer: Pausing video to check if OK to play")

        # Prompt the user for the pin, returns True if they knew it
        pinResult = KeltecPinGuard.promptUserForPin(securityLevel)
        if pinResult is None:
            self.lastMovieSetChecked = ""
            xbmc.executebuiltin("Action(Back)", True)
            xbmc.sleep(300)
            xbmc.executebuiltin("Action(Back)", True)
        elif pinResult:
            log("KeltecPinGuardPlayer: Resuming video")
            # Pausing again will start the video playing again
            self.pause()
        else:
            log("KeltecPinGuardPlayer: Stopping video")
            self.stop()
            KeltecPinGuard.displayInvalidPinMessage(securityLevel)

        xbmcgui.Window(10000).clearProperty("KeltecPinGuardPrompting")
...
class NavigationRestrictions():
    ...
    def checkTvShows(self):
        ...
        # Check if we have already cached the pin number and at which level
        if KeltecPinGuard.getCachedPinLevel() >= securityLevel:
            log("NavigationRestrictions: Already cached pin at level %d, allowing access" % KeltecPinGuard.getCachedPinLevel())
            return
    ...
    def checkPlugins(self):
        ...
        # Detect if this is the Keltec Pin Guard addon itself
        # Match by path (ID) OR by display name - Container.FolderName returns "Keltec Pin Guard"
        isOurAddon = ('script.keltecpinguard' in navPath) or                      ('keltec pin guard' in pluginName.lower()) or                      ('keltecpinguard' in pluginName.lower())

        # If we reach here we have a Plugin that we need to check
        log("NavigationRestrictions: Checking access to view Plugin: %s (isOurAddon=%s)" % (pluginName, isOurAddon))
        self.lastPluginChecked = pluginName

        # Check for the case where the user does not want to check plugins
        # but the Keltec Pin Guard plugin is selected - we ALWAYS protect it
        if (not Settings.isActivePlugins()) and (not isOurAddon):
            return
        ...
        if securityLevel < 1:
            if isOurAddon:
                # Always require PIN to access the guard's own settings
                securityLevel = Settings.getSettingsSecurityLevel()
                if securityLevel < 1:
                    # If no PIN set yet, level is 0 - nothing to protect
                    log("NavigationRestrictions: No PIN configured yet, allowing access to guard settings")
                    return
            else:
    ...
    # Checks to see if the KeltecPinGuard addons screen has been opened
    def checkSettings(self):
        ...
        # Check if the addon is the KeltecPinGuard addon
        addonId = xbmc.getInfoLabel("ListItem.Property(Addon.ID)")
        if 'script.keltecpinguard' not in addonId:
            self.canChangeSettings = 0
            return
        ...
        pinDB = KeltecPinGuardDB()
        securityLevel = pinDB.getPluginSecurityLevel('KeltecPinGuard')
        del pinDB
        ...
        if pinResult is None:
            ...
            if addonSettings:
                ...
                xbmc.executebuiltin("Addon.OpenSettings(script.keltecpinguard)", False)
        else:
    ...
    # Checks to see if the KeltecPinGuard is being requested to be shown
    def checkForcedDisplay(self):
        # Check if the property is set
        if xbmcgui.Window(10000).getProperty("KeltecPinGuardPrompt") != 'true':
            return

        # Set the lowest security level for the forced display
        securityLevel = 1

        # Close any open dialogs before showing pin
        xbmc.executebuiltin("Dialog.Close(all, true)", True)
        ...
        if pinResult is None:
            ...
            xbmcgui.Window(10000).clearProperty("KeltecPinGuardPrompt")
        else:
            log("NavigationRestrictions: Not allowed access after forced lock at security level %d" % securityLevel)
            # The pin dialog will be automatically re-opened as the window property has not been cleared
...
class UserPinControl():
    ...
    def startupCheck(self):
        ...
        while tryAgain:
            ...
            if self.userId in [None, ""]:
                ...
                msg = "%s\n%s" % (ADDON.getLocalizedString(32104), ADDON.getLocalizedString(32129))
                tryAgain = xbmcgui.Dialog().yesno(ADDON.getLocalizedString(32001), msg)
        ...
        if preventAccess:
            ...
            xbmcgui.Window(10000).setProperty("KeltecPinGuard_RestrictedUser", self.userId)
    ...
    def checkDisplayStatus(self):
        ...
        if xbmcgui.Window(10000).getProperty("KeltecPinGuard_DisplayStatus") not in ["", None]:
            xbmcgui.Window(10000).clearProperty("KeltecPinGuard_DisplayStatus")
            self.displaySummary()
    ...
    def shutdown(self, reason=0):
        ...
        if reason > 0:
            cmd = 'Notification("{0}", "{1}", 3000, "{2}")'.format(ADDON.getLocalizedString(32001), ADDON.getLocalizedString(reason), ICON)
            xbmc.executebuiltin(cmd)

        # Give the user time to see why we are shutting down
        waitTime = 35
        while (waitTime > 0) and (not systemMonitor.abortRequested()):
            xbmc.sleep(100)
            waitTime = waitTime - 1
...
##################################
# Main of the KeltecPinGuard Service
##################################
if __name__ == '__main__':
    log("Starting Keltec Pin Guard Service %s" % ADDON.getAddonInfo('version'))
    ...
    playerMonitor = KeltecPinGuardPlayer()
    systemMonitor = KeltecPinGuardMonitor()
    navRestrictions = NavigationRestrictions()
    pvrMonitor = PvrMonitor()

    loopsUntilUserControlCheck = 0
    while (not systemMonitor.abortRequested()):
        ...
        # a sleep state, if restoring from sleep in memory then the KeltecPinGuard
        ...
        if (lastActivityTime == 0) or ((lastActivityTime + 10) < currentTime):
            ...
            if Settings.isPromptForPinOnStartup():
                log("KeltecPinGuard: Prompting for pin on startup")
                xbmcgui.Window(10000).setProperty("KeltecPinGuardPrompt", "true")
        ...
        # Check if the Pin is set, as no point prompting if it is not
        if KeltecPinGuard.isKeltecPinGuardEnabled():
            ...
            # permissions using the KeltecPinGuard plugin
    ...
    log("Stopping Keltec Pin Guard Service")
    del pvrMonitor
    del userCtrl
    del navRestrictions
    del playerMonitor
