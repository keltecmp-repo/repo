...
ICON_DIR = xbmcvfs.translatePath(os.path.join(CWD, 'resources', 'media', 'classifications'))
...
class MenuNavigator():
    ...
    def _setList(self, target):
        ...
        elif target == MenuNavigator.REPOSITORIES:
            items = self._setRepositoryList()
        else:
        ...
        for item in items:
            ...
            li.setArt({'icon': item['thumbnail'], 'thumb': item['thumbnail']})
    ...
    def _addSecurityFlags(self, type, items):
        ...
        elif type == MenuNavigator.REPOSITORIES:
            securityDetails = pinDB.getAllRepositoriesSecurity()
        ...
        for item in items:
            ...
            if item['title'] in securityDetails:
                ...
                log("KeltecPinGuardPlugin: %s has security level %d" % (title, securityLevel))
    ...
    def _cleanClassification(self, target, items):
        ...
        log("KeltecPinGuardPlugin: Allowing certificates for %s" % str(certValues))
            ...
            if 'mpaa' in item:
                if item['mpaa'] not in [None, ""]:
                    ...
                    try:
                        ...
                        log("KeltecPinGuardPlugin: Failed to encode certificate")
                    ...
                    try:
                        ...
                        log("KeltecPinGuardPlugin: Failed to decode title")

                    if cert in certValues:
                        item['mpaa'] = cert
                        log("KeltecPinGuardPlugin: Setting mpaa for %s to %s" % (title, cert))
                    else:
                        log("KeltecPinGuardPlugin: Clearing mpaa for %s (was %s)" % (title, item['mpaa']))
                        item['mpaa'] = ""
        return items
    ...
    def _setPluginList(self):
        ...
        if ("result" in json_response) and ('addons' in json_response['result']):
            ...
            for addonItem in json_response['result']['addons']:
                ...
                if addonId in ['script.keltecpinguard']:
                    log("setPluginList: Skipping KeltecPinGuard Plugin")
                    continue
    ...
    # Get the list of repos installed on the system
    ...
    def _setClassificationList(self, type="", subtype=""):
        ...
        if type in [None, ""]:
            ...
            try:
                ...
                log("KeltecPinGuardPlugin: Failed to sort language list")
            ...
            for classification in classifications:
                ...
                if idStr in securityDetails:
                    ...
                    log("KeltecPinGuardPlugin: Classification %s has security level %d" % (fullName, securityLevel))
    ...
    def setSecurity(self, type, title, id, oldLevel, classBlocked=False, forceLevel=None):
        ...
        if title not in [None, ""]:
            ...
            if type == MenuNavigator.TVSHOWS:
                ...
                pinDB.setRepositorySecurityLevel(title, id, level)
            elif type == MenuNavigator.FILESOURCE:
    ...
    def _setBulkSecurity(self, type, level):
        ...
        for item in items:
            ...
            try:
                ...
                log("KeltecPinGuardPlugin: setBulkSecurity Failed to encode title %s" % title)
...
################################
# Main of the KeltecPinGuard Plugin
################################
if __name__ == '__main__':
    ...
    log("KeltecPinGuardPlugin: Called with addon_handle = %d" % addon_handle)

    # If None, then at the root
    if mode is None:
        log("KeltecPinGuardPlugin: Mode is NONE - showing root menu")
        menuNav = MenuNavigator(base_url, addon_handle)
        menuNav.showRootMenu()
        del menuNav

    elif mode[0] == 'folder':
        log("KeltecPinGuardPlugin: Mode is FOLDER")
        ...
        log("KeltecPinGuardPlugin: Mode is SET SECURITY")
        ...
        if (type is not None) and (len(type) > 0):
            log("KeltecPinGuardPlugin: Type to set security for %s" % type[0])
            secTitle = ""
        ...
        log("KeltecPinGuardPlugin: Mode is FORCE PIN")
        xbmcgui.Window(10000).setProperty("KeltecPinGuardPrompt", "true")

    elif mode[0] == 'setclassification':
        log("KeltecPinGuardPlugin: Mode is SET CLASSIFICATION")
